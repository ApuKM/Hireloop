...
export default function PostJobForm({ company }: { company: CompanyData }) {
  ...
  const normalizedStatus =
    company.status?.toLowerCase() as keyof typeof STATUS_CONFIG;
  const currentStatus =
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="bg-[#18181b] border border-zinc-800 rounded-xl mb-8 p-4 flex flex-row items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-white">
                Posting as <span className="font-bold">{company.formName}</span>
              </p>
              <div className="flex items-center gap-2 mt-1">
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium border ${currentStatus.className}`}
                >
                  {currentStatus.icon}
                  <span>{currentStatus.label}</span>
                </span>
                {/* <span className="text-xs text-zinc-400">
                  {plan.name} Plan ({applications.length}/
                  {plan.maxApplicationsPerMonth} jobs used)
                </span> */}
              </div>
            </div>
          </div>
          ...
          {company.status !== "approved" && (
            <div className="text-yellow-100">Please wait to get approved!</div>
          )}
        </div>
